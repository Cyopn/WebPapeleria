...
export default function DocsPage() {
    ...
    const calculatePrice = useCallback(async (uploadInfo, overrides = {}) => {
        ...
        try {
            ...
            } catch (e) { console.error('[DocsPage] fallo parseando usuario en localStorage', e) }
            ...
            try {
                ...
                setUploads((prev) => {
                    ...
                    if (merged.length > 0) {
                        ...
                        try { calculatePrice(merged) } catch (err) { console.error('[DocsPage] calculatePrice fallo', err) }
                    }
                    return merged
                })
    ...
    function removeUpload(idx) {
        setUploads((prev) => {
            ...
            try { calculatePrice(next) } catch (err) { console.error('[DocsPage] calculatePrice fallo', err) }
            return next
        })
    }
                    ...
                    <div className='w-full h-[100%] bg-white z-[2] flex justify-center items-center end-0'>
                        <div className='absolute flex flex-col items-center top-[43%] w-[85%]'>
                                ...
                                <div className='w-[80%] p-2 text-black grid grid-cols-[repeat(3,1fr)] grid-rows-[1fr] gap-y-[10px]'>
                                    <div className='w-full p-2'>
                                        ...
                                        <div className='w-full px-2 pb-2'>
                                            ...
                                            <div className='w-full flex flex-col items-center content-stretch justify-center'>
                                                <select id='paper' name='paper' value={bothSides ? 'ac' : 'oc'} onChange={(e) => { const v = e.target.value === 'ac'; setBothSides(v); if (uploads.length) calculatePrice(uploads, { bothSides: v }) }} className='w-full border rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 bg-[#D9D9D9] border-gray-600 placeholder-gray-400 text-black focus:ring-blue-500 focus:border-blue-500'>
                                    ...
                                    <div className='w-full p-2'>
                                        ...
                                        <div className='w-full flex flex-col items-center content-stretch justify-center'>
                                            <select id='bound' name='bound' value={docType} onChange={(e) => { const v = e.target.value; setDocType(v); if (uploads.length) calculatePrice(uploads, { docType: v }) }} className='w-full border rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 bg-[#D9D9D9] border-gray-600 placeholder-gray-400 text-black focus:ring-blue-500 focus:border-blue-500'>
                            ...
                            <PaymentModal
                                ...
                                currency={'MXN'}
                                context={{ lastUpload, uploads, printType, paperSize, rangeValue, bothSides, quantity, priceData, docType, deliveryDate, observations }}
