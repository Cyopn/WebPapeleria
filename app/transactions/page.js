        ...
        <section className='relative top-[104px] min-h-[calc(100vh-104px)] w-full bg-gradient-to-br from-[#F2F8FF] via-[#FFFFFF] to-[#EEF4FF] p-5 md:p-8 text-black'>
            <div className='mx-auto w-full max-w-6xl'>
                <div className='mb-6 flex flex-wrap items-center justify-between gap-3'>
                    <div>
                        <h1 className='text-2xl md:text-3xl font-semibold'>Historial de pedidos</h1>
                        <p className='text-sm text-gray-600'>Consulta el detalle de tus compras y su estado actual.</p>
                    </div>
                </div>
                ...
                <div className='mb-4 flex flex-wrap items-center gap-3'>
                    <input
                        ...
                        className='w-full max-w-sm border rounded-lg block p-2.5 border-gray-600 placeholder-gray-400 text-black'
                    />

                    <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className='rounded-lg border px-3 py-3 text-sm border-gray-600 placeholder-gray-400 text-black cursor-pointer'>
                        <option value=''>Todos los estados</option>
                        <option value='pending'>Pendiente</option>
                        <option value='completed'>Completado</option>
                    </select>

                    <select value={paymentFilter} onChange={(e) => setPaymentFilter(e.target.value)} className='rounded-lg border px-3 py-3 text-sm border-gray-600 placeholder-gray-400 text-black cursor-pointer'>
                    ...
                    <div className='flex items-center gap-2'>
                        <DatePicker
                            ...
                            className='rounded-lg border px-2 py-3 text-sm border-gray-600 placeholder-gray-400 cursor-pointer'
                        />
                        <DatePicker
                            selected={dateTo}
                            ...
                            className='rounded-lg border px-2 py-3 text-sm border-gray-600 placeholder-gray-400 text-black cursor-pointer'
                        />
                    </div>
                    <button type='button' onClick={() => { setStatusFilter(''); setPaymentFilter(''); setDateFrom(null); setDateTo(null); setQ(''); }} className='rounded-full border px-3 py-2 text-sm cursor-pointer'>Limpiar</button>
                </div>
                ...
                <div className='mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4'>
                    ...
                    <div className='rounded-2xl border border-[#D8E8FF] bg-white p-4 shadow-sm'>
                        ...
                        <p className='text-2xl font-semibold text-amber-600'>{summary.pendientes}</p>
                    </div>
                    <div className='rounded-2xl border border-[#D8E8FF] bg-white p-4 shadow-sm'>
                        <p className='text-xs text-gray-500'>Completados</p>
                        <p className='text-2xl font-semibold text-emerald-600'>{summary.completados}</p>
                    </div>
                    <div className='rounded-2xl border border-[#D8E8FF] bg-white p-4 shadow-sm'>
